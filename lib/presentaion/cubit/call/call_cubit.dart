...
class CallCubit extends Cubit<CallState> {
  ...
  Future<void> playContactingRing({required bool isCaller}) async {
    String audioAsset = "sounds/ringlong.mp3";
    assetsAudioPlayer.setSourceAsset(audioAsset);
    //ByteData bytes = await rootBundle.load(audioAsset);
    //Uint8List  soundBytes = bytes.buffer.asUint8List(bytes.offsetInBytes, bytes.lengthInBytes);
    assetsAudioPlayer.play(AssetSource("sounds/ringlong.mp3"));
/*
...
*/
  ...
  Future<void> updateCallStatusToAccept({required CallModel callModel}) async {
    ...
    initAgoraAndJoinChannel(
        ...
        isVideo: callModel.isVideo?? false,
        isCaller: false);
  }
